(ns kajabi.main
  "Kotodama WASM entrypoint for the Kajabi clean-room actor (L4) — Clojure port.
  ...
  facts (`kajabi.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kajabi")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"kajabi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kajabi-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kajabi-compat/ready)
